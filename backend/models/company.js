...
module.exports.updateUserDetails = function(user, callback) {
  ...
  var filter = {
    arrayFilters: [{ 'i._id': user._id }]
  };

  Company.update(query, data, filter, callback);
};

// Adding to status array
module.exports.addStatues = function(companyID, status, callback) {
  var query = { _id: companyID };
  Company.findOneAndUpdate(query, { $pull: { statues: status } }, callback);
};
