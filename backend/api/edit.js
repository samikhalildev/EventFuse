  ...
  Event.updateEvent(req.body.eventID, req.body, function(err, doc) {
    ...
    Company.getAllUserCompaniesByID(user._id, function(err, companies) {
      ...
      res.render('dashboard', {
        companies: companies,
        eventUpdated: req.body.eventID,
        success_msg: 'Event has been updated.'
      });
    });
  });
  /*
...
// DELETE event
router.get('/delete/:companyID/:eventID', ensureAuthenticated, function(
  req,
  res
) {
  let companyID = req.params.companyID;
          ...
          Company.getEvent(companyID, eventID, function(err, event) {
            ...
            Event.removeEvent(eventID, function(err, doc) {
              if (err) {
                ...
                Company.removeEventFromCompany(companyID, eventID, function(
                  err,
                  done
                ) {
                  if (err) throw err;
...
function isMatch(search, query) {
  for (var i = 0; i < search.length; i++) {
    if (search[i].toString() == query.toString()) return true;
  }
  return false;
}
